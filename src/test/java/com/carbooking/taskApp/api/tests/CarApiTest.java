...
import io.qameta.allure.restassured.AllureRestAssured; // <-- Импорт фильтра Allure
...
public class CarApiTest extends BaseApiTest{
    ...
    public void testGetCarTypes_ShouldReturn200() {
        ...
        Response response = RestAssured
                .given()
                .filter(new AllureRestAssured()) // <-- Фильтр добавлен
    ...
    public void testGetCars_WithDateFilter_ShouldShowCarsFreeOnDates() {
        ...
        Response response = RestAssured.given() // <-- Добавлен given()
                .filter(new AllureRestAssured()) // <-- Фильтр добавлен
                .when()
